...
TEST_CASE("HCIFormatExtractor with event packet", "[unit][format][hci]") {
  ...
  */
  ...
  REQUIRE(address == array<uint8_t, 6>{0x63, 0xc0, 0x76, 0xf4, 0xfb, 0xe7}); // It is normal that the address is reversed (due to HCI endianness)
...
TEST_CASE("HCIFormatBuilder", "[unit][format][hci]") {
  ...
  hci_builder
      .set_opcode(Format::HCI::CommandCode::SetScanParameters)
